...
public class NemicoManagerLV2b : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        enemycode1 = gameObject.GetComponent<LV2b_Enemy_Generator>().GetEnemyCode();
       // Debug.Log("EnemyCode 2b: " + enemycode1);

        //prendo soluzione
        soluzione = Convert.ToString(DataBase.Rows.Find(enemycode1)[2]);
        ...
        if (soluzione.Substring(1, 1) == schermoColpito)
        {
            if (tagArma == soluzione.Substring(0, 1) && attivati == false)
            {
                ...
                if (vitaModuloSigla <= 0)
                {
                    ...
                    char[] moduli = enemycode1.ToCharArray();
                    int materialmodulo1 = gameObject.GetComponent<LV2b_Enemy_Generator>().DecodeM1(Convert.ToString(moduli[1]));
                ...
                if (vitaModuloLinea <= 0)
                {
                    ...
                    char[] moduli = enemycode1.ToCharArray();
                    int materialmodulo2 = gameObject.GetComponent<LV2b_Enemy_Generator>().DecodeM2(Convert.ToString(moduli[2]));
